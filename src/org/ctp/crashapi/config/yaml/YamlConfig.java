...
public class YamlConfig {
	...
	public YamlConfig(File configFile, String[] header) {
		this.header = header;
		file = configFile;
		if (configFile != null) config = YamlConfiguration.loadConfiguration(configFile);
	...
	public List<String> getEnums(String path) {
		if (enums.containsKey(path)) return enums.get(path);
		return null;
	}
	...
	public void addComments(String path, String[] values) {
		if (path.startsWith("config_comments.")) path = path.substring("config_comments.".length());
	...
	public String getType(String path) {
		...
		if (info == null) return "nested value";

		if (getEnums(path) != null) {
			if (info.getStringList() != null) return "enum_list";
			return "enum";
		}
		...
		if (info.getDoubleValue() != null) return "double";

		if (info.getStringList() != null) return "list";

		if (info.getString() != null) return "string";
	...
	public Object get(String path) {
		if (info.containsKey(path)) return info.get(path).getValue();
		return null;
	}

	public Location getLocation(String path) {
		YamlInfo info = getInfo(path);
		if (info == null || info.getString() == null) return null;
		...
		Location location = new Location(w, x.doubleValue(), y.doubleValue(), z.doubleValue(), yaw, pitch);
		return location;
	}
	...
	public Object get(String path, Object def) {
		if (get(path) != null) return get(path);
		return def;
	}
	...
	public String getString(String path, String def) {
		YamlInfo info = getInfo(path);
		if (info == null || info.getString() == null) return def;
		return info.getString();
	}
	...
	public boolean containsElements(String path) {
		if (getInfo(path) != null) return true;
		for(Iterator<java.util.Map.Entry<String, YamlInfo>> it = info.entrySet().iterator(); it.hasNext();) {
			java.util.Map.Entry<String, YamlInfo> e = it.next();
			if (e.getKey().startsWith(path)) return true;
		}
		return false;
	}
	...
	public List<String> getLevelEntryKeys(String level) {
		...
		for(Iterator<java.util.Map.Entry<String, YamlInfo>> it = info.entrySet().iterator(); it.hasNext();) {
			...
			if (level == null || level.equals("")) {
				for(int i = 0; i < entryKeys.size(); i++)
					if (!entryKeys.get(i).contains(".") && !values.contains(entryKeys.get(i))) values.add(entryKeys.get(i));
			} else
				for(int i = 0; i < entryKeys.size(); i++)
					if (entryKeys.get(i).startsWith(level + ".")) if (entryKeys.get(i).length() > level.length() + 1) {
						String find = entryKeys.get(i).substring(level.length() + 1);
						if (!find.contains(".") && !values.contains(entryKeys.get(i))) values.add(entryKeys.get(i));
					}
		}
		return values;
	}
	...
	protected String getLevel(YamlChild child, boolean comments) {
		...
		if (contains(key)) {
			...
			if (objects != null) {
				...
				Object o = get(key);
				if (o instanceof String) line.append("'" + ((String) o).replaceAll("'", "''") + "'");
				else
					line.append(o);
			}
		}
	...
	public void addMinMax(String path, int i, int j) {
		if (getInfo(path) == null) defaults.get(path).setMinMax(i, j);
		else
