...
public class PacketNMS extends NMS {
	...
	public static int addParticle(Block block, int stage) {
		switch (getVersionNumber()) {
			...
			default:
				if (isSimilarOrAbove(getVersionNumbers(), 1, 18, 0)) return Packet_2.addParticle(block, stage);
	...
	public static int updateParticle(Block block, int stage, int id) {
		switch (getVersionNumber()) {
			...
			default:
				if (isSimilarOrAbove(getVersionNumbers(), 1, 18, 0)) return Packet_2.updateParticle(block, stage, id);
